...
app.post('/dados-sensores', (req, res) => {
    ...
    db.run(`INSERT INTO dados_sensores (sensor_id, temperatura, umidade, vibracao, tensao) VALUES (?, ?, ?, ?, ?)`, 
           [dados.sensor_id, dados.temperatura, dados.umidade, dados.vibracao, dados.tensao], 
           (err) => {
               if (err) {
...
app.post('/inserir-dados-sensor', (req, res) => {
    const { sensor_id, temperatura, umidade, vibracao, tensao } = req.body;
    console.log('Dados do sensor recebidos:', { sensor_id, temperatura, umidade, vibracao, tensao });

    // Lógica para atualizar os dados no banco de dados
    db.run(`INSERT INTO dados_sensores (sensor_id, temperatura, umidade, vibracao, tensao) VALUES (?, ?, ?, ?, ?)`, 
           [sensor_id, temperatura, umidade, vibracao, tensao], 
           (err) => {
               if (err) {
                   console.error('Erro ao inserir dados no banco de dados:', err.message);
                   res.status(500).send('Erro ao processar os dados.');
